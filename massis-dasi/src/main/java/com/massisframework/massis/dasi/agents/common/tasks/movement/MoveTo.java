...
public class MoveTo
		implements AgentTask<LocationHolder, MoveTo.ApproachResult> {
	...
	public CompletionStage<ApproachResult> execute(
			RuleHighLevelController rlhc, LocationHolder loc) {
		CompletableFuture<ApproachResult> cf = new CompletableFuture<MoveTo.ApproachResult>();
		rlhc.getLowLevelAgent().approachTo(loc.getLocation(),
